      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-xl p-8 text-center">
          <p className="text-gray-400 text-sm font-semibold">
            No hay sugerencias {filter !== 'all' ? `con estado "${statusLabel[filter]}"` : ''}
          </p>
        </div>
      ) : (
            ...
            <div key={s.id} className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  ...
                  {s.status === 'approved' && (
                    <span className="text-xs text-green-600 font-bold">Agregado a la biblioteca</span>
                  )}
                  {s.status === 'rejected' && (
                    <button onClick={() => updateStatus(s.id, 'pending')}
                      className="px-3 py-2 bg-white text-gray-400 border border-gray-200 text-xs font-black uppercase tracking-widest rounded hover:border-gray-300 transition-all whitespace-nowrap">
                      ↩ Restaurar
                    </button>
                  )}
                </div>
